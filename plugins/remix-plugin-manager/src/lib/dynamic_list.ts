...
export class Item<I> extends theia.TreeItem {
...
export class List<I> implements theia.TreeDataProvider<I> {
    private list: I[]
    private options = { idKey: 'id', labelKey: 'label' }
    public render = new theia.EventEmitter<I>()
    ...
    getTreeItem(element: I): Item<I> {
        if (element) {
            const label = (element as any)[this.options.labelKey]
            const item = new Item(label, this.name, element)
            return item
        }
        throw new Error('element is not defnied');
    }
...
export class DynamicListPlugin<I, T extends List<I> = List<I>> extends Plugin {
    ...
    protected treeView: theia.TreeView<I>
    ...
    activate() {
        ...
        const { idKey, labelKey } = this.options
        this.list.setOptions({ idKey, labelKey })
        this.treeView = theia.window.createTreeView(`${this.profile.name}`, { treeDataProvider: this.list })
        this.listeners = [
            this.treeView,
            theia.commands.registerCommand(`${this.profile.name}.select`, item => this.select(item)),
            theia.commands.registerCommand(`${this.profile.name}.reset`, items => this.reset(items)),
            theia.commands.registerCommand(`${this.profile.name}.add`, item => this.add(item)),
            theia.commands.registerCommand(`${this.profile.name}.remove`, (id) => this.remove(id)),
            theia.commands.registerCommand(`${this.profile.name}.update`, (id, item) => this.update(id, item)),
        ]
        super.activate()
    }
    ...
    reset(items: I[]) {
        ...
        items.forEach((item, i) => {
            const id = this.options.idKey ? ((item as any)[this.options.idKey] || i) : i

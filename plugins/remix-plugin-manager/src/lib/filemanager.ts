...
export class FileManagerPlugin extends CommandPlugin implements MethodApi<IFileSystem> {
    ...
    async readFile(path: string): Promise<string> {
        const absPath = absolutePath(path);
        const uri = theia.Uri.file(absPath);
        return theia.workspace.fs.readFile(uri).then((content: any) => Buffer.from(content).toString("utf-8"));
    }
    ...
    async readdir(path: string): Promise<string[]> {
        const absPath = absolutePath(path);
        const uri = theia.Uri.file(absPath);
        return theia.workspace.fs.readDirectory(uri).then((data: any) => data.map(([path]: any[]) => path));
    }

...
const pluginProfile = [
    {
        ...
        methods: [],
        version: '0.0.1',
        documentation: 'https://github.com/protofire/remix-solhint-plugin',
        url: 'https://protofire.github.io/remix-solhint-plugin/',
    ...
    },
    {
        ...
        version: '0.0.1',
        documentation: 'https://github.com/protofire/remix-solhint-plugin',
        url: 'https://ipfsgw.komputing.org/ipfs/QmYCUgBadbcHj3B8d8TsT4A2fKYvXGFWt1izxyXasjdWtF',
        description: 'Run Solhint in your Remix project',
        icon: 'https://raw.githubusercontent.com/protofire/solhint/master/solhint-icon.png',
        location: 'sidePanel'
    },
    {
        name: "chainlens",
        displayName: "ChainLens",
        events: [],
        methods: [],
        url: "https://chainlens.pipeos.one",
...
class RemixPlugin {
    ...
    async togglePlugin() {
        this.toggleThisPlugin(pluginProfile[0])
        this.toggleThisPlugin(pluginProfile[1])
        // this.toggleThisPlugin(pluginProfile[2])
        // this.toggleThisPlugin(pluginProfile[3])
    }
    async toggleThisPlugin(profile: Profile & ExternalProfile) {
        const name = profile.name
        if (!this.engine.isRegistered(name)) {
            const webview = new WebviewPlugin({ ...profile }, {
                context: this.context
            }, () => this.manager.call('manager', 'deactivatePlugin', name));
            this.engine.register(webview)
        }
        if (await this.manager.isActive(name)) {
            await this.manager.deactivatePlugin(name)
        } else {
            await this.manager.activatePlugin(name)
        }
        theia.window.showInformationMessage('plugin is active: ' + await this.manager.isActive(name))
    }
}
